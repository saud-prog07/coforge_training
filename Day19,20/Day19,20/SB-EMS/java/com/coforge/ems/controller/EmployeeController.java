...
public class EmployeeController {
	...
	public ResponseEntity<?> findDepartmentById(@PathVariable("dno") int dno) {
		...
		DepartmentDTO department = service.getDepartmentById(dno);

		if (department != null) {
			return new ResponseEntity<>(department, HttpStatus.OK);
		} else {
			return new ResponseEntity<>("FAILED : Department object not found", HttpStatus.BAD_REQUEST);
		}
	}
	...
	public ResponseEntity<EmployeeDTO> findEmployeeDetails(@Valid @PathVariable int eid) {

		ResponseEntity<EmployeeDTO> responseEntity = null;
		
		EmployeeDTO employeeDto = new EmployeeDTO();
		
		Employee employees = service.findByEid(eid).get();
		DepartmentDTO department = service.getDepartmentById(employees.getDno());
		
		employeeDto.setEmployee(employees);
